...
class RHKsm4:
    ...
    def __init__(self, filename):
        ...
        offset = self._page_index_header._get_offset(
    ...
    def _reads(self, count):
        ...
        string = "".join(
            [
                chr(i)
                for i in np.fromfile(self._file, dtype=np.uint16, count=count)
            ]
        )
    ...
    def _readstr(self):
        ...
        length = self._readb(
            np.uint16, 1
        )  # first 2 bytes is the string length
        string = "".join(
            [
                chr(i)
                for i in np.fromfile(self._file, dtype=np.uint16, count=length)
            ]
        )
        try:
            string.encode("utf8")
...
class RHKObjectContainer:
    ...
    def _read_StringData(self, offset):
        ...
        for k in range(self._string_count):
            if k == 4:  # file path
                ...
                for i, CH in enumerate(CHlist):
                    self.attrs["RHK_CH" + str(i + 1) + "DriveValue"] = float(
                        CH.split(" ")[3]
                    )
                    self.attrs[
                        "RHK_CH" + str(i + 1) + "DriveValueUnits"
                    ] = CH.split(" ")[4]
            else:
        ...
        if self.attrs["RHK_Xlabel"] == "":
            self.attrs["RHK_Xlabel"] = line_type_xunits[
                self.attrs["RHK_LineTypeName"]
            ]
    ...
    def _read_SpecDriftHeader(self, offset):
        ...
        self.attrs["RHK_SpecDrift_DriftOptionType"] = self._sm4._readb(
            np.uint32, 1
        )
        self.attrs["RHK_SpecDrift_DriftOptionTypeName"] = drift_option_type(
            self.attrs["RHK_SpecDrift_DriftOptionType"]
        ).name
    ...
    def _read_SpecDriftData(self, offset):
        ...
        for k in range(self.attrs["RHK_Ysize"]):
            self.attrs["RHK_SpecDrift_Time"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_SpecDrift_Xcoord"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_SpecDrift_Ycoord"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_SpecDrift_dX"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_SpecDrift_dY"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_SpecDrift_CumulativeX"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_SpecDrift_CumulativeY"].append(
                self._sm4._readb(np.float32, 1)
            )
    ...
    def _read_ImageDriftHeader(self, offset):
        ...
        self.attrs["RHK_ImageDrift_DriftOptionType"] = self._sm4._readb(
            np.uint32, 1
        )
        self.attrs["RHK_ImageDrift_DriftOptionTypeName"] = drift_option_type(
            self.attrs["RHK_ImageDrift_DriftOptionType"]
        ).name
    ...
    def _read_ImageDrift(self, offset):
        ...
        self.attrs["RHK_ImageDrift_CumulativeX"] = self._sm4._readb(
            np.float32, 1
        )
        self.attrs["RHK_ImageDrift_CumulativeY"] = self._sm4._readb(
            np.float32, 1
        )
    ...
    def _read_ColorInfo(self, offset):
        ...
        for k in range(self._color_info_count):
            self.attrs["RHK_Color_StructSize"].append(
                self._sm4._readb(np.uint16, 1)
            )
            self.attrs["RHK_Color_Reserved"].append(
                self._sm4._readb(np.uint16, 1)
            )

            ## HSVColor
            self.attrs["RHK_Color_Hstart"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_Color_Sstart"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_Color_Vstart"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_Color_Hstop"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_Color_Sstop"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_Color_Vstop"].append(
                self._sm4._readb(np.float32, 1)
            )

            self.attrs["RHK_Color_ClrDirection"].append(
                self._sm4._readb(np.uint32, 1)
            )
            self.attrs["RHK_Color_NumEntries"].append(
                self._sm4._readb(np.uint32, 1)
            )
            self.attrs["RHK_Color_StartSlidePos"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_Color_EndSlidePos"].append(
                self._sm4._readb(np.float32, 1)
            )

            ## Color Transform
            self.attrs["RHK_Color_Gamma"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_Color_Alpha"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_Color_Xstart"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_Color_Xstop"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_Color_Ystart"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_Color_Ystop"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_Color_MappingMode"].append(
                self._sm4._readb(np.uint32, 1)
            )
            self.attrs["RHK_Color_Invert"].append(
                self._sm4._readb(np.uint32, 1)
            )
    ...
    def _read_TipTrackHeader(self, offset):
        ...
        self.attrs["RHK_TipTrack_FeatureHeight"] = self._sm4._readb(
            np.float32, 1
        )
        self.attrs["RHK_TipTrack_FeatureWidth"] = self._sm4._readb(
            np.float32, 1
        )
        self.attrs["RHK_TipTrack_TimeConstant"] = self._sm4._readb(
            np.float32, 1
        )
        ...
        self.attrs["RHK_TipTrack_TipTrackInfoCount"] = self._sm4._readb(
            np.uint32, 1
        )
    ...
    def _read_TipTrackData(self, offset):
        ...
        for k in range(self.attrs["RHK_TipTrack_TipTrackInfoCount"]):
            self.attrs["RHK_TipTrack_CumulativeTime"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_TipTrack_Time"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_TipTrack_dX"].append(
                self._sm4._readb(np.float32, 1)
            )
            self.attrs["RHK_TipTrack_dY"].append(
                self._sm4._readb(np.float32, 1)
            )
    ...
    def _read_PRMdata(self, offset):
        ...
        if self._PRM_CompressionFlag == 0:
            PRMdata = np.fromfile(
                self._sm4._file, dtype=np.uint32, count=self._PRM_DataSize
            )
        elif self._PRM_CompressionFlag == 1:
            comprPRMdata = np.fromfile(
                self._sm4._file,
                dtype=np.uint32,
                count=self._PRM_CompressionSize,
            )
            PRMdata = zlib.decompress(
                comprPRMdata, wbits=0, bufsize=self._PRM_DataSize
            )
    ...
    def _read_PiezoSensitivity(self, offset):
        ...
        self.attrs["RHK_PiezoSensitivity_TubeX"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_PiezoSensitivity_TubeY"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_PiezoSensitivity_TubeZ"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_PiezoSensitivity_TubeZOffset"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_PiezoSensitivity_ScanX"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_PiezoSensitivity_ScanY"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_PiezoSensitivity_ScanZ"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_PiezoSensitivity_Actuator"] = self._sm4._readb(
            np.float64, 1
        )
        ...
        self.attrs[
            "RHK_PiezoSensitivity_TubeZOffsetUnit"
        ] = self._sm4._readstr()
        ...
        self.attrs[
            "RHK_PiezoSensitivity_TubeCalibration"
        ] = self._sm4._readstr()
        self.attrs[
    ...
    def _read_FrequencySweepData(self, offset):
        ...
        self.attrs["RHK_FrequencySweep_PSDTotalSignal"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_FrequencySweep_PeakFrequency"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_FrequencySweep_PeakAmplitude"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_FrequencySweep_DriveAmplitude"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_FrequencySweep_Signal2DriveRatio"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_FrequencySweep_QFactor"] = self._sm4._readb(
            np.float64, 1
        )
        ...
        self.attrs[
            "RHK_FrequencySweep_PeakFrequencyUnit"
        ] = self._sm4._readstr()
        self.attrs[
            "RHK_FrequencySweep_PeakAmplitudeUnit"
        ] = self._sm4._readstr()
        self.attrs[
            "RHK_FrequencySweep_DriveAmplitudeUnit"
        ] = self._sm4._readstr()
        self.attrs[
            "RHK_FrequencySweep_Signal2DriveRatioUnit"
        ] = self._sm4._readstr()
    ...
    def _read_ScanProcessorInfo(self, offset):
        ...
        self.attrs["RHK_ScanProcessor_XSlopeCompensation"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_ScanProcessor_YSlopeCompensation"] = self._sm4._readb(
            np.float64, 1
        )
    ...
    def _read_PLLInfo(self, offset):
        ...
        self.attrs["RHK_PLL_DriveRefFrequency"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_PLL_LockinFreqOffset"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_PLL_LockinHarmonicFactor"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_PLL_LockinPhaseOffset"] = self._sm4._readb(
            np.float64, 1
        )
        ...
        self.attrs["RHK_PLL_DissPIIntCutoffFreq"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_PLL_DissPILowerBound"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs["RHK_PLL_DissPIUpperBound"] = self._sm4._readb(
            np.float64, 1
        )
    ...
    def _read_ChannelDriveInfo(self, offset, metaString):
        ...
        self.attrs[metaString + "_MasterOscillator"] = self._sm4._readb(
            np.uint32, 1
        )
        ...
        self.attrs[metaString + "_PhaseOffset"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs[metaString + "_HarmonicFactor"] = self._sm4._readb(
            np.float64, 1
        )
    ...
    def _read_LockinInfo(self, offset, metaString):
        ...
        self.attrs[metaString + "_NonMasterOscillator"] = self._sm4._readb(
            np.uint32, 1
        )
        ...
        try:
            self.attrs[
                metaString + "_FilterCutoffFrequency"
            ] = self._sm4._readstr()
    ...
    def _read_PIControllerInfo(self, offset, metaString):
        ...
        self.attrs[metaString + "_ProportionalGain"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs[metaString + "_IntegralGain"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs[metaString + "_LowerBound"] = self._sm4._readb(
            np.float64, 1
        )
        self.attrs[metaString + "_UpperBound"] = self._sm4._readb(
            np.float64, 1
        )
...
class RHKPageHeader(RHKObjectContainer):
    ...
    def read_sequential_type(self, page):
        ...
        page._page_data_size = page.attrs["RHK_ParamCount"] * (
            page.attrs["RHK_DataLength"] + 1
        )
    ...
    def read_objects(self, page):
        ...
        if page._page_data_type == 6:
            ...
            for i in range(page.attrs["RHK_ParamCount"]):
                ## Parameter gain
                page.attrs["RHK_Sequential_ParamGain"].append(
                    self.sm4._readb(np.float32, 1)
                )
                ## Name of the parameter
                page.attrs["RHK_Sequential_ParamLabel"].append(
                    self.sm4._readstr()
                )
                ## Unit of the parameter
                page.attrs["RHK_Sequential_ParamUnit"].append(
                    self.sm4._readstr()
                )
...
class RHKPage(RHKObjectContainer):
    ...
    def _read_data(self):
        ...
        if (
            self._line_type in [1, 6, 9, 10, 11, 13, 18, 19, 21, 22]
            or self._page_data_type == 6
        ):
            raw_data = np.fromfile(
                self._sm4._file, dtype=np.float32, count=data_size
            )
            ...
            raw_data = np.fromfile(
                self._sm4._file, dtype=np.int32, count=data_size
            )
    ...
    def _reshape_data(self, raw_data):
        ...
        if self._page_data_type == 0:  # Image type
            ...
            coords = [
                (x_label, abs(xscale) * np.arange(xsize, dtype=np.float64)),
            ...
            coords = [
                (
                    x_label,
                    xscale * np.arange(xsize, dtype=np.float64) + xoffset,
                ),
            ...
            if (
                self._line_type == 22
            ):  # Discrete spectroscopy has shape xsize*(ysize+1)
